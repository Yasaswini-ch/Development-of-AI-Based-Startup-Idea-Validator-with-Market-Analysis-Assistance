...
4. **Market Opportunity & Competitor Discovery Agents** (Milestone 2) — two more agents
   that run after the Web Search step, reasoning over its real results (context
...
See `agent/output_guard.py`'s `strip_reasoning()`, used by the Market Opportunity and
...
| Agents | CrewAI | Role/goal-based agent definitions, one Agent+Task per pipeline stage. Only 2 agents left: Market Opportunity and Competitor Discovery, both taking real data as context rather than using a tool (tool-calling proved to be the source of the reasoning-leak bug). The Web Search step used to be a third CrewAI agent too, but its LLM call was cut entirely - see Reasoning LLM below. |
...
- M2 agents (Market Opportunity, Competitor Discovery) validate their JSON shape after
  balanced-brace extraction: if it parses to valid JSON with every required field of
  the right type, it's used regardless of any scratchpad rambling around it (past
  `strip_reasoning()` - see `agent/output_guard.py`); otherwise the node fails and
  reports through `errors.<node>` rather than trusting malformed output
- LLM rate-limit fallback: every remaining crew (market opportunity, competitor
  ...
  rebuilds it against the next model in the fallback chain instead of waiting — a
  separate quota bucket on the same account (see Tech Stack Decisions). Only once every
  model in the chain has been rate limited does it wait out the last one's suggested
  cooldown (capped at 30s) for one final try. A non-rate-limit exception, or a failure
  after every model and the final retry,
